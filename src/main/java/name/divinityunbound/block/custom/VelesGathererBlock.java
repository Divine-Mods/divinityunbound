...
public class VelesGathererBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new VelesGathererBlockEntity(pos, state);
    }
    ...
    private void updateUpgrades(World world, BlockPos pos) {
        ...
        if (be instanceof VelesGathererBlockEntity) {
            ((VelesGathererBlockEntity)be).resetUpgrades();
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof VelesGathererBlockEntity) {
                ItemScatterer.spawn(world, pos, ((VelesGathererBlockEntity)blockEntity).internalInventory);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            NamedScreenHandlerFactory screenHandlerFactory = ((VelesGathererBlockEntity) world.getBlockEntity(pos));
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.VELES_GATHERER_BLOCK_ENTITY,
